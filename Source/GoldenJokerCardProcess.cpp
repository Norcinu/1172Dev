...
void GoldenJokerCardProcess::Update()
{
	...
	if (mCardIndex < 5)
	{
		if (POKER_GAME->PokerHand[mCardIndex].id == GOLDEN_JOKER_CARD)//JOKER_CARD)
		{			
			...
			mCardDelayTimer = ENGINE->GetSystemTimer().GetRunningTime() + POKER_GAME->GeneralSetGameDelay(0.1f);
		mCardIndex++;
	}		
}